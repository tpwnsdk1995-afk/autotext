...
중요 — 행 개수는 실제 이미지와 정확히 일치해야 합니다 (많아도 적어도 안 됨):
- 이미지 안에서 온전히 다 보이는 데이터 행 하나당 정확히 JSON 항목 하나를 만드세요. 행을 빠뜨리거나 중복으로 만들지 마세요.
- 다만 (경계에서 잘린 게 아니라) 온전히 보이는 행인데 글씨가 흐려서 이름이나 번호를 확실히 읽을 수 없으면, 절대 지어내지 말고 그 필드만 빈 문자열("")로 남기세요 — 행 자체는 반드시 포함해야 합니다.
- "수령유형", "인천반납" 같은 열 이름/라벨 문구 자체를 name으로 쓰지 마세요. 확실한 이름이 안 보이면 name을 빈 문자열로 두세요.
